  ...
  return (
    <div className="flex flex-col max-h-fit max-w-64 min-w-64 text-main-font rounded-lg border border-gray-300 bg-[#C5AD891A] shadow">
      ...
      <div className="flex flex-col items-center py-8">
        ...
        {/* Profile Image */}
        <img
          src={isAuthUser?.image === undefined ? isAuthUser?.user?.image: isAuthUser?.image} // Replace with the actual image URL
          alt="Profile"
          className="w-20 h-20 rounded-full mb-4"
        />
        {/* Name */}
        <h2 className="text-lg font-semibold">{name === undefined ? updatename : name}</h2>
      </div>
      ...
      <nav className="flex flex-col py-4 px-6">
        <div className="flex flex-col items-center space-y-6">
          ...
          >
            <span className="w-12 inline-flex justify-end">
              <Newspaper className={`w-5 h-5 duration-300 ${location.pathname === "/article" ||location.pathname ===  "/article/featured-articles"? "text-main" : ""}`}/>
            </span>
            <span className={`ml-4 duration-300 ${location.pathname === "/article" ||location.pathname ===  "/article/featured-articles" ? "text-main" : ""} `}>Article</span>
          </Link>
          <Link
            to="/tags"
            className="flex items-center text-sm font-medium hover:text-main w-40"
          >
            <span className="w-12 inline-flex justify-end">
              <Tag  className={`w-5 h-5 duration-300 ${location.pathname === "/tags" ? "text-main" : ""}`}/>
            </span>
            <span className={`ml-4 duration-300 ${location.pathname === "/tags" ? "text-main" : ""} `}>Tags</span>
          </Link>
          <Link
            to="/activities"
            className="flex items-center text-sm font-medium hover:text-main w-40"
          >
            <span className="w-12 inline-flex justify-end">
              <Calendar className={`w-5 h-5 duration-300 ${location.pathname === "/activities" ? "text-main" : ""}`}/>
            </span>
            <span className={`ml-4 duration-300 ${location.pathname === "/activities" ? "text-main" : ""} `}>Activities</span>
          </Link>
          ...
          >
            <span className="w-12 inline-flex justify-end">
              <Mail className={`w-5 h-5 duration-300 ${location.pathname === "/messages" ? "text-main" : ""}`}/>
            </span>
            <span className={`ml-4 duration-300 ${location.pathname === "/messages" ? "text-main" : ""} `}>Messages</span>
          </Link>
          <Link
            to="/admin"
            className="flex items-center text-sm font-medium hover:text-main w-40"
          >
            <span className="w-12 inline-flex justify-end">
              <User className={`w-5 h-5 duration-300 ${location.pathname === "/admin" ? "text-main" : ""}`}/>
            </span>
            <span className={`ml-4 duration-300 ${location.pathname === "/admin" ? "text-main" : ""} `}>Admins</span>
          </Link>
        </div>
